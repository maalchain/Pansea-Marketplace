...
contract MedinaNFTMarketplace is Pausable, ReentrancyGuard {
    ...
    mapping(address => uint256) public collectionIdToNFTAddress;
    ...
    function listNFT(
        ...
        collectionIdToNFTAddress[NFTContractAddress] = collectionId;
    ...
    function createOfferWithMAAL(
        ...
        require(msg.value == offerAmount, "Send right amount for offer");

        // Transfer offer amount to contract owner
        (bool offerSuccess, ) = address(this).call{value: msg.value}("");
        require(offerSuccess, "Platform fee transfer failed");
        ...
        offer.tokenAdd = IERC20(address(0x0));
    ...
    function buyNFTWithMAAL(uint256 _listingId, uint256 _quantity)
        external
        payable
        nonReentrant
    {
        ...
        require(
            msg.value == listing.PricePerNFT * _quantity,
            "Send right amount to purchase"
        );
        ...
        require(royaltyFeeSuccess, "Royalty transfer failed");
        // Transfer the rest to the seller
        (bool sellerAmntSuccess, ) = listing.seller.call{value: sellerValue}(
            ""
        );
        require(sellerAmntSuccess, "Seller amount transfer failed");
        ...
        if (isERC721) {
            ...
            // Transfer NFT from seller to buyer
            IERC1155 NFTContract = IERC1155(listing.NFTContractAddress);
            uint256 quantityToTransfer = _quantity; // Store _quantity in a temporary variable
            uint256 _tokenIdToTransfer = listing.TokenId;
            NFTContract.safeTransferFrom(
                listing.seller,
                msg.sender,
                _tokenIdToTransfer,
                quantityToTransfer, // Use the temporary variable here
                ""
            );
        }
        ...
        nonReentrant
    {
        ...
        require(
            settlementToken.transferFrom(msg.sender, owner, platformFeeValue),
            "Failed to transfer platform fee"
        );
        require(
            settlementToken.transferFrom(
